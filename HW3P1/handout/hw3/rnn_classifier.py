    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            ...
            for j, rnn_cell in enumerate(self.rnn):
                h_state = self.hiddens[-1][j]
                x_cur = rnn_cell.forward(x_cur, h_state)
                cur += [x_cur]
            self.hiddens.append(cur.copy())

        # Get the outputs from the last time step using the linear layer and return it
        # <--------------------------
        logits = self.output_layer.forward(x_cur)
        # return logits
        return logits
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for i in range(seq_len-1, -1, -1):
            for j, rnn_cell in reversed(list(enumerate(self.rnn))): # going through layers 
                ...
                new_dx, dh[j] = rnn_cell.backward(delta, h, h_prev_l, h_prev_t)

                if j > 0: # not the first layer 
                    dh[j-1] += new_dx

        # return dh / batch_size
        return dh / batch_size # dh_0
