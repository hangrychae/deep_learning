...
class CTC(object):
    ...
    def get_forward_probs(self, logits, extended_symbols, skip_connect):
        ...
        logits: (np.array, dim = (input_len, len(Symbols)))
                ...
                p(t,s(i)) = logits[t, gextSymbols[i]]
    ...
    def get_backward_probs(self, logits, extended_symbols, skip_connect):
        ...
        for t in range(T-2, -1, -1):
            beta[t][S-1] = beta[t+1][S-1] * logits[t, extended_symbols[-1]]
    ...
    def get_posterior_probs(self, alpha, beta):
        ...
        for t in range(0, T):
            ...
            for j in range(0, S):
                gamma[t][j] = gamma[t][j] / sumgamma[t]
        return gamma
...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            target_b = self.target[batch_itr][:target_lengths[batch_itr]]
            logits_b = self.logits[:,batch_itr,:][:input_lengths[batch_itr],:]
            extended_symbols, skip_connect = self.ctc.extend_target_with_blank(target_b)
            alpha = self.ctc.get_forward_probs(logits_b, extended_symbols, skip_connect)
            beta = self.ctc.get_backward_probs(logits_b, extended_symbols, skip_connect)
            ...
            for t in range(T):
                for r in range(R):
                    div -= gamma[t,r] * np.log(logits_b[t, extended_symbols[r]])
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            # TODO
            # <---------------------------------------------
            target_b = self.target[batch_itr][:self.target_lengths[batch_itr]]
            logits_b = self.logits[:,batch_itr,:][:self.input_lengths[batch_itr],:]
            extended_symbols, skip_connect = self.ctc.extend_target_with_blank(target_b)
            alpha = self.ctc.get_forward_probs(logits_b, extended_symbols, skip_connect)
            beta = self.ctc.get_backward_probs(logits_b, extended_symbols, skip_connect)
            gamma = self.ctc.get_posterior_probs(alpha, beta)
            ...
            for t in range(T):
                dY[t,batch_itr,:] = 0
                for i in range(R):
                    dY[t, batch_itr, extended_symbols[i]] -= gamma[t,i] / logits_b[t, extended_symbols[i]]
