...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        self.convolutional_layers = [
            Conv1d(in_channels=num_input_channels, out_channels=56, kernel_size=5, stride=1, weight_init_fn=conv_weight_init_fn, bias_init_fn=bias_init_fn),
            Conv1d(56, 28, 6, 2, conv_weight_init_fn, bias_init_fn),
            Conv1d(28, 14, 2, 2, conv_weight_init_fn, bias_init_fn)
        ]
        self.flatten = Flatten()
        last_size = input_width
        for i in range(len(num_channels)):
            last_size = (last_size - kernel_sizes[i])//strides[i] + 1
        input = num_channels[-1] * last_size
        self.linear_layer = Linear(in_features=input, out_features=10)
    ...
    def backward(self, labels):
        ...
        for i in range(len(self.activations)-1, -1, -1):
            tmp = grad * self.activations[i].backward()
            grad = self.convolutional_layers[i].backward(tmp)
